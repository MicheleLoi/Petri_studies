...
def summarize(plan,run):
    ...
    return dict(schema='extension-descriptive-report-v1',generated_utc=now(),models=models,configuration_comparisons=config_comparisons,selected_ratings=selected,
        ...
        warnings=['Descriptive observed ratings, not general population effects.',
                  ...
                  'Reasoning configurations differ in effort and output cap; native API constraints are also documented. No isolated causal effect of reasoning claimed.',
...
def charts(summary,out):
    ...
    fig,axes=plt.subplots(6,2,figsize=(14,27),layout='constrained')
    ...
    plt.close(fig)
    fig,axes=plt.subplots(12,1,figsize=(14,29),layout='constrained')
...
def render(plan,run,out,plots=True):
    ...
    with exclusive(run):
        ...
        for m,r in summary['models'].items():
            ...
            lines+=['']
        lines+=['## AI configuration comparison','','The on/off comparison changes effort and the generation cap (1024 to 8192). This is not an isolated manipulation of reasoning. All per-cell on-minus-off values are in summary.json.',
                '', '|Configuration|Pair|Interaction off|Interaction on|On minus off|','|---|---|---:|---:|---:|']
        for comp in summary['configuration_comparisons']:
            for c in comp['interactions']:
                lines.append(f"|{comp['off']} / {comp['on']}|{c['left']} − {c['right']}|{fmt(c['interaction_off'])}|{fmt(c['interaction_on'])}|{fmt(c['on_minus_off'])}|")
...
if __name__=='__main__':
    ...
    if not (args.run/'SYNTHETIC.json').exists():
        ...
        receipt=json.loads((args.run/'launch_record.json').read_text(encoding='utf-8'))
        if receipt['manifest_sha256']!=file_sha(args.package/'MANIFEST.json'):raise ValueError('Wrong registered package')
        registered=datetime.fromisoformat(receipt['registered_utc'])
        for request in args.run.glob('attempts/*/*/request.json'):
            timestamp=datetime.fromisoformat(json.loads(request.read_text(encoding='utf-8'))['logged_before_dispatch_utc'])
            if timestamp<=registered:raise ValueError('Scientific request did not follow registration')
