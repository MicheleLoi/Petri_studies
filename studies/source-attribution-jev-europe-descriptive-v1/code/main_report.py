...
def summarize(plan,run):
    ...
    return dict(schema='jev-europe-descriptive-report-v1',generated_utc=now(),models=models,configuration_comparisons=config_comparisons,selected_ratings=selected,
...
def render(plan,run,out,plots=True):
    ...
    with exclusive(run):
        ...
        title='SYNTHETIC REHEARSAL — NOT STUDY DATA' if summary['synthetic'] else 'Jev Germany and Switzerland: descriptive supplement'
