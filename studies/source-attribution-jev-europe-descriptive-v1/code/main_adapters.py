...
def assess(model, envelope, plan):
    ...
    if d is None:
        ...
        return r
    try:
        if model != 'jev': raise ValueError('Only Jev is supported')
        if model == 'jev':
            ...
            single_upstream = type(routing.get('totalProviderAttemptCount')) is int and routing['totalProviderAttemptCount']==1
            version = d.get('model')
            ...
            try:
                raw_cost = gateway.get('cost')
                if type(raw_cost) not in (int,float,str): raise ValueError('Invalid gateway cost type')
                cost = float(raw_cost)
                if number(cost): r['billing_estimate_usd'] = cost
            except (ValueError, TypeError, OverflowError): pass
            r['cost_basis'] = 'gateway reported cost; invoice unverified'
            r['pause_model'] = not identity or not legend or consistent is False or not single_upstream
        if not r['identity_accepted']: r['flags'].append('model_identity_unverified_or_changed')
        r['category']='usable' if r['rating_usable'] else 'missing_rating'
...
class CredentialStore:
    """Read the existing local key only when preparing an authorized send."""
...
class HttpTransport:
    ...
    def send(self, spec, auth, timeout):
        if spec['endpoint']!='https://ai-gateway.vercel.sh/typesafe/v1/systemone' or spec['provider']!='vercel_typesafe':
            raise ValueError('Unexpected Jev endpoint/provider')
