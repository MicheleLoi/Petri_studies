...
def verify_package(package):
    ...
    if len(plan['schedule'])!=1024 or len({r['slot'] for r in plan['schedule']})!=1024:
        raise ValueError('Invalid planned slots')
...
def verify_launch(package, receipt):
    ...
    required=('registration_reference','registered_utc','visibility','author_launch_instruction',
              'jev_version_recheck_utc','jev_version_evidence')
    if any(not isinstance(r.get(k),str) or not r[k].strip() for k in required):
        raise ValueError('Actual preregistration reference, launch instruction and dated Jev recheck required')
    if r.get('manifest_sha256')!=digest or r.get('jev_presumed_model')!=plan['jev_presumed_version']:
        raise ValueError('Receipt does not cover this frozen package/version')
    for field in ('registered_utc','jev_version_recheck_utc'):
    ...
    if datetime.fromisoformat(r['jev_version_recheck_utc']).date()!=datetime.now(timezone.utc).date():
        raise ValueError('Recheck Jev identity evidence on actual launch/resume date')
...
class Engine:
    ...
    def suspend(self, model, reason):
        with self.mutex:
            self.paused.add(model)
            p=self.run/'suspensions'/f'{model}.json'
            if not p.exists(): save(p,dict(model=model,reason=reason,timestamp_utc=self.clock()))
    ...
    def slot(self,row):
        ...
        while len(h)<3 and m not in self.paused and not self.stop.is_set():
            ...
            with self.mutex:
                if not self.budget_check(self.plan['reserve_eur_per_attempt']):
                ...
                req=dict(row=row,attempt=n,spec=spec,spec_sha256=sha(spec),
                    logged_before_dispatch_utc=self.clock(),reserved_eur=self.plan['reserve_eur_per_attempt'])
                save(p,req)  # irrevocable attempt reservation before any send
            timeout=(self.plan['timeout_connect_seconds'], self.plan['jev_timeout_read_seconds'] if m=='jev' else self.plan['timeout_read_seconds'])
            try:
            ...
            with ThreadPoolExecutor(max_workers=self.plan['parallel_workers']) as pool:
                for block in range(1,self.plan['blocks']+1):
                    ...
                    for row in rows:
                        ...
                        if not any(r['rating_usable'] for r in h): planned+=(3-len(h))*self.plan['reserve_eur_per_attempt']
                    ...
                    futures=[pool.submit(self.model_block,[r for r in rows if r['model']==m]) for m in self.plan['models']]
